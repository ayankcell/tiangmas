<template>
    <div class="p-4">
        ...
        <div class="gap-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-1">
            <NuxtLink :to="post._path" v-for="post of data" :key="post._path" class="lg:flex lg:items-center lg:gap-2" :title="post.title">
...
<script setup>
const { data } = await useAsyncData('recentPosts', () => queryContent('artikel').sort({ date: -1 }).limit(8).find())
</script>